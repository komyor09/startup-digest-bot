...
MVP.
---
### Сохранение данных (MVP)

В проекте используется SQLite без постоянного хранилища.
При развертывании во временных средах база данных может быть пересоздана при перезапуске.

Такое поведение допустимо на этапе MVP и задокументировано намеренно.
В производственной среде следует использовать постоянное хранилище или управляемую базу данных.
---
## Будущие улучшения
- Стратегии резервного копирования для конкретных источников (API / рендеринг HTML, где это разрешено)
- Более продвинутая оценка релевантности (модели на основе NLP)
